...
export class Layouter {
  ...
  layoutCollaboration(collaboration, options = {}) {
    ...
    // leave room above first participant for collaboration-level annotations
    let currentY = 60;
    ...
    for (const { participant, process, layoutedProcesses, grid } of participantLayouts) {
      ...
      if (grid) {
        ...
        // Draw flow elements with participant offset
        const shift = { x: PARTICIPANT_LABEL_WIDTH, y: currentY };
  ...
  generateArtifactsDi(process, procDi) {
    ...
    // Position text annotations above their source element
    textAnnotations.forEach(annotation => {
      const association = associations.find(
        assoc => assoc.targetRef === annotation || assoc.sourceRef === annotation
      );

      // Annotations with no association default to origin
      let x = 0, y = 0;
      ...
      if (association) {
        ...
        if (peer && peer.di) {
          ...
          const { height } = getDefaultSize(annotation);
          x = peerBounds.x;
          y = peerBounds.y - height - 20;
        }
      }
